...
keywords = ['-c-cocoa-', '-c-sdpop-', '-dcop-base-']

for kwd in tqdm.tqdm(keywords):
    data_files = [f for f in os.listdir('../metrics') if kwd in f and '.csv' in f]

    if data_files:
        dataframes = [pd.read_csv(f'../metrics/{metrics_file}') for metrics_file in data_files]
